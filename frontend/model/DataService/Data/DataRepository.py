...
class DataRepository:
    ...
    def insert_rows_into_table(self, table_name, columns, rows, schema_name="Warehouse"):
        if(len(columns) == 0):
        ...
        for row in rows:
            ...
            if(len(accumulator) == self.rows_per_insert or len(accumulator) == len(rows)):
                counter = counter + 1
                print(f'Creating insert statement #{counter}')
                queries.append(self.__create_insert_statement__(table_name, columns, accumulator, schema_name))
                accumulator = []
        counter = 0
    ...
    def __create_insert_statement__(self, table_name, columns, rows, schema_name):
